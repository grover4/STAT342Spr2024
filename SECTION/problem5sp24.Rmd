...
$$f(x) = \theta \frac{1}{(1+x)^{\theta+1}} \ \ x \geq 0$$
where $\theta$ is known to be 2,3 or 4.
...
3. A company has manufactured certain objects and has printed a serial number on each manufactured object. The serial numbers start at 1 and end at $N$, where $N$ is the (unknown) number of objects that have been manufactured. One of these objects is selected at random, and the serial number of that object is $x_{obs} = 888$.
    ...
    Let $X$ denote the number which is randomly chosen. Then
    $$f(x)  = \frac{1}{N},  \ \ \ x=1,2,\dots,N$$

a. Derive the method of moments estimate of $N$? (Hint: Find $E\left[X\right]$)


b. Derive the likelihood function of $N$. (Hint: this is like  example 24.3 in the slidedeck labeled Chapter 24)

c. Derive the maximum likelihood estimate of $N$? 

d. Contrast the two methods. What is an advantage of the method of moments over the maximum likelihood? How about a disadvantage? (There are no right or wrong answers)  

4. Suppose 100 seeds are planted and it is known only that $x \leq 15$ seeds germinated. That is, the exact number of germinating seeds is not known. Let $\pi$ denote the probability that a randomly selected seed will germinate. 
